...
export class LinkCodesController {
  ...
  async create(@Body() body: CreateLinkCodeDto, @Req() req): Promise<LinkCodeDto> {
    return this.sequelize.transaction(async (transaction) => {
      const service = body.fileId ? this.filesService : this.foldersService;
      ...
      const linkCode = await this.linkCodesService.create(body, req.user.id, transaction);
